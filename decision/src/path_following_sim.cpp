...
#define  K          (1.0)       // coefficient of front view distance
...
void global_path(const nav_msgs::Path::ConstPtr& msg, ros::Publisher chatter_pub)
{
  ...
  Point a;
	...
	for(i=0;i<(msg->poses.capacity());i++)
	{
		a.x = msg->poses[i].pose.position.x;
		a.y = msg->poses[i].pose.position.y;
	...
	//	printf("path=%f,%f\n",path[i].x,path[i].y);
		...
		chatter_pub.publish(coordinate);
...
int main(int argc,char **argv)
{
  ...
  ros::Publisher chatter_pub = n.advertise<geometry_msgs::Point>("coordinate", 1000);
  ros::Subscriber sub_coordinate = n.subscribe<nav_msgs::Path>("/move_base/GlobalPlanner/plan", 1000, boost::bind(&global_path, _1, chatter_pub));
 ...
 while(ros::ok())
 {
  ...
  {
    if(flag2==1){
      rearIndex = path.size() - 1;
      goalIndex = get_goal_index(state,path);
      flag2 = 0;
    }
